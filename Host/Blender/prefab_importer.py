...
from ...RuriRipperPyBridge.unity import (asset_db, clip_curves, clip_paths as clip_repair,
                                         discovery, mesh_decoder, prefab as prefab_scan,
                                         skinning)
from ...RuriRipperPyBridge.unity.hierarchy import ANIMATOR_ROOT_PATH
from ...Kernel import options as kernel_options
from ...Kernel.app import prefabs as app_prefabs

UNITY_TAG = "unity_tag"

#: Every switch a caller can hand in. The user-facing ones come from the ONE
#: schema (Kernel/options.py) rather than being restated here -- a second copy is
#: how Painter ended up with a "LOD0 only" tick whose key nothing downstream read.
#:
#: The rest are below: values this importer is TOLD rather than asked. The four
#: import_* among them are NOT preferences and are deliberately not offered
#: anywhere -- a model without its materials or its skeleton is a broken import,
#: not a lighter one. They exist because a handful of internal paths state a
#: narrower intent: cross_game_retarget loads a clip's authoring rig purely to
#: read its rest pose and has no use for materials or clips, and a story stage
#: places geometry under an existing rig.
...
def import_prefab_from_db(context, db, prefab_file, options=None, name=None,
    ...
    if options["import_animations"]:
        report.available_clips = discovery.discover_clip_refs(db)
    return report
...
# What a game states about its OWN prefabs -- a renderer whose geometry is
# attached at run time, a detail level encoded somewhere other than a LODGroup --
# is a fact about that game rather than about this application, so the
# declaration is the kernel's and this reads it. Named here because every game
# module that registers one has always reached for it through this module.
MESH_RESOLVERS = app_prefabs.MESH_RESOLVERS
DETAIL_RULES = app_prefabs.DETAIL_RULES
register_mesh_resolver = app_prefabs.register_mesh_resolver
unregister_mesh_resolver = app_prefabs.unregister_mesh_resolver
register_detail_rule = app_prefabs.register_detail_rule
unregister_detail_rule = app_prefabs.unregister_detail_rule
...
def _detail_test(db, prefab, options):
    ...
    stated = app_prefabs.detail_test(db, prefab, level, options)
...
def _renderer_mesh_ref(db, prefab_file, renderer, options):
    ...
    stated = app_prefabs.mesh_ref(db, prefab_file, renderer, options)
    return stated if stated else renderer.mesh_ref
...
def _maps_from_armature(arm_obj):
    ...
    # A root bone's own name is not in its path, so its path is the empty string -- the
    # animator root's path, and the binding a clip's extracted root motion arrives on. It
    # names one bone only while this armature has a single root; several roots leave it
    # unbound rather than guessing which one the clip meant.
    single_root = sum(1 for bone in arm_obj.data.bones if bone.parent is None) == 1
    ...
    for bone in arm_obj.data.bones:
        ...
        if node.path != ANIMATOR_ROOT_PATH or single_root:
