...
def build_mesh_object(context, decoded, name, armature_obj, smr_bones,
    ...
    if decoded.normals is not None:
        ...
        try:
            # A LIST, not the array: the setter walks its argument through the Python
            # sequence protocol, and handing it the ndarray costs 7.7s over one level
            # (measured) for the copy tolist() avoids.
    ...
    # validate() answers whether it CHANGED the mesh; when it did not, the corner->vertex
    # map built above still describes it and the bake need not read one back.
    rebuilt = mesh.validate(clean_customdata=False)
    mesh.update()

    _bake_tangents(mesh, decoded, None if rebuilt else loop_verts)
...
def _bake_tangents(mesh, decoded, loop_verts=None):
    ...
    # ``loop_verts`` arrives only when validate() reported the mesh untouched; the length
    # check is the backstop, because a stale map is a silent length mismatch that Blender
    # answers with "internal error setting the array" and takes the whole import down with.
    if loop_verts is None or len(loop_verts) != len(mesh.loops):
