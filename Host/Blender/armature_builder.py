...
def build_armature(context, unity_file, name="UnityArmature"):
    ...
    # A node a clip can bind curves onto. The ANIMATOR ROOT is one of them: its path is the
    # empty string, and that is the path a clip's extracted root motion -- the travel a
    # humanoid solve splits out of RootT/MotionT -- is bound by, so testing a path for
    # truthiness here is what silently drops a character's whole travel. It names one
    # transform only while the prefab has a single root; several disjoint trees have no one
    # animator root, and binding an arbitrary one of them would be a guess.
    single_root = len(roots) == 1

    def bindable(node, bone_name):
        if bone_name is None:
            return False
        return single_root or node.path != hierarchy.ANIMATOR_ROOT_PATH

    path_to_bone = {}
    for node in nodes.values():
        bone_name = file_id_to_bone.get(node.file_id)
        if bindable(node, bone_name):
...
def build_armature_from_avatar(context, avatar_file, name="Avatar"):
    ...
    avatar document (UNITY_AVATAR_PROP) are both stamped, so any clip --
    generic or muscle-encoded -- binds to it standalone. The bones themselves are
    build_armature_from_nodes' work; this reads the avatar's skeleton for it and
    stamps the document afterwards."""
    try:
    ...
    arm_obj, _bone_names = build_armature_from_nodes(context, raw_nodes, name)
...
def build_armature_from_nodes(context, raw_nodes, name="Armature"):
    ...
    source's own order, parents anywhere in the list, in the same space every decoded mesh
    arrives in. Returns the armature object and ``{index: bone name}`` -- the names may have
    been uniquified, and a caller binding weights needs the ones that stuck.

    This is where an armature is built. build_armature_from_avatar is Unity's door onto it;
    a producer that states its skeleton outright comes straight here.
    """
    from mathutils import Quaternion
    ...
    # that is ``path is None``, and it is NOT the same thing as the animator
    # root, whose path is the empty string and which a clip's root motion binds
    # by (see hierarchy.ANIMATOR_ROOT_PATH). The root names one transform only
    # while this skeleton has a single root.
    single_root = len(roots) == 1
    stamped = {}
    for index, (_bone_name, _parent, _t, _q, path) in enumerate(raw_nodes):
        bone_name = index_to_bone_name.get(index)
        if not bone_name or path is None:
            continue
        if path == hierarchy.ANIMATOR_ROOT_PATH and not single_root:
            continue
        stamped[path] = {"bone": bone_name,
                         "local": [v for row in locals_[index] for v in row]}
    stamp_rig(arm_obj, stamped)
    return arm_obj, index_to_bone_name
