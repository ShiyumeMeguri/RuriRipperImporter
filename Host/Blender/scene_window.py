"""Build a whole scene window into the scene.
...
What a placement is, and which columns state it, are the GAME's facts, and they
reach here as ``Packages`` of kind SCENE_WINDOW -- a columnar table plus the
container paths whose closure holds what those rows draw. Nothing below names a
game, reads a game module, or asks the decoder anything: the closure crossing it
rides (import_reachable) knows no game either.
"""
...
def console_lines(report, closure):
    """Everything about one window import that belongs in the console.

    An UNRESOLVED placement is the one number worth acting on, and a path alone is
    not actionable -- "why did it not join" is. Three questions answer it outright
    and are asked here rather than by whoever reads the log later: is the container
    path in the cabmap at all, which CAB holds it, and was that CAB in this
    window's closure. Missing any one of them means guessing, and guessing once
    cost a whole re-import."""
    lines = [report.summary(), "phases: " + report.phase_summary()]
    lines.extend(report.warnings)
    if report.no_geometry:
        lines.append(
            "{0} placement(s) of {1} asset(s) carry no geometry in the game's own data "
            "(collision proxies) -- nothing was lost: {2}".format(
                report.no_geometry, len(report.no_geometry_assets),
                ", ".join(report.no_geometry_assets[:5])))
    if report.unresolved_materials:
        lines.append(
            "{0} material path(s) did not resolve -- those slots are EMPTY on the meshes "
            "that named them: {1}".format(len(report.unresolved_materials),
    ...
    for path in report.unresolved_paths:
        ...
        elif not any(cab in closure for cab in cabs):
...
def lost_content(report):
    """What the PANEL has to say rather than print. Exactly one thing qualifies:
    an unresolved placement is content that is missing from the scene, and burying
    that in a count is how three missing foliage imposters hid for a whole
    session."""
    if not report.unresolved:
        return []
    return ["{0} placement(s) of {1} asset(s) did NOT resolve and are MISSING "
            "(paths in the console).".format(report.unresolved,
                                             len(report.unresolved_paths))]
...
def import_scene_window(context, bridge, window, options=None):
    """Import one stated window (``Packages.window``).
            ...
            if builtin is not None:
                ...
                if mat_builder is not None:
                    for material_path in materials_by_row.get(row, ()):
                        material_guid = guids.get(material_path) if material_path else None
                        materials.append(
                            _timed_material(mat_builder, {"guid": material_guid}, report)
                            if material_guid is not None else None)
                ...
                materials = []
                if mat_builder is not None:
                    # Positional: build_mesh_object takes one entry per sub-mesh and
                    # accepts None for a slot, so a material that does not resolve
                    # leaves ITS slot empty instead of pulling every later sub-mesh
                    ...
                    for material_path in materials_by_row.get(row, ()):
                        ...
                        if material_guid is None:
                            ...
                            continue
                        materials.append(_timed_material(mat_builder, {"guid": material_guid}, report))
