...
# Property names no layer of the role table states, gathered while building, live in
# the kernel: BOTH hosts meet them (a name no layer states is a fact about the game's
# vocabulary, not about node graphs), and the panel that lists them is one panel.
from ...Kernel.app import texturing as _texturing

_UNRESOLVED = _texturing.UNRESOLVED
...
def _content_digest(props):
    """A stable digest of what a material DECLARES -- the shader it names and every
    property table this builder reads -- independent of which guid, CAB or package
    carries it. json with sorted keys over the normalised tables, so two materials
    that resolve to the same parameters digest equally regardless of key order, and
    a streaming map that stamps one material into hundreds of chunks builds it once.

    Digesting the PROPERTIES rather than the document they were read from is what
    lets a producer with no document (see MaterialBuilder.build_from_props) share
    the same deduplication."""
    import hashlib
    import json

    payload = {
        "shader": props.shader_ref if props.shader_ref is not None else props.shader_name,
        "name": props.name,
        "textures": props.textures,
        "texture_st": props.texture_st,
        "floats": props.floats,
        "colors": props.colors,
        "keywords": sorted(props.keywords),
        "disabled_passes": props.disabled_passes,
    }
...
def _image_from_texture_bytes(data, name, extension=None):
...
def _exported_extension(db, key):
    """The container this texture was exported in, as its own exported path states it, or None
    when nothing states one. The exporter chose that extension; reading it back beats making
    Blender detect the format off the image."""
    path = db.asset_path(key) if hasattr(db, "asset_path") else None
...
def _image_extension(image):
    ...
    if remembered is not None:
        return remembered
    _EXTENSION_BY_FORMAT[detected] = extension = _read_image_extension(image, detected)
    return extension
...
class MaterialBuilder:
    ...
    def shader_display_name(self, props):
        ...
        (same PBR model)."""
        # A producer that KNOWS the name states it (an engine whose material names its
        # own base material); nothing then has to be looked up to answer.
        if props.shader_name:
            return props.shader_name
        ...
        name = unity_material.shader_identity(self.db.raw_text(key))
        self._shader_names[key] = name
        return name

    def build_from_ref(self, ref):
        """Build/return a material from a {fileID, guid} reference: the Unity door
        onto build_from_props, which is where a material is actually built."""
        ...
        if doc is None:
            ...
            _announce(mat)
            return mat
        return self.build_from_props(unity_material.parse_material(doc), guid)

    def build_from_props(self, props, key):
        """Build/return a material from a property set already read, under ``key`` --
        whatever the producer calls this material (a Unity guid, an engine's object path).

        This is the entry point; build_from_ref is the Unity reference in front of it.
        A producer that reads its materials some other way arrives here with the same
        normalised tables and gets the same graph, built by the same code.

        Two cache layers, both identity-true: the key (this exact material), then a
        digest of what the material itself declares. Content-equal materials build one
        Blender material, not hundreds of identical ones."""
        if key in self._cache:
            return self._cache[key]
        digest = _content_digest(props)
        mat = self._content_cache.get(digest)
        if mat is None:
            mat = self._build(props)
            mat[UNITY_GUID_PROPERTY] = str(key)
    ...
    def _load_image(self, guid, non_color=False):
        ...
        if cached is None:
            if hasattr(self.db, "texture_bytes"):
                ...
                cached = _image_from_texture_bytes(
                    data, self.db.asset_name(key) or key, _exported_extension(self.db, key))
            else:
