...
def build_action(clip, armature_obj, maps, path_to_meshobjects=None, options=None,
    ...
    # Build a path -> node lookup for rest transforms. The ANIMATOR ROOT is in here: its
    # path is the empty string (hierarchy.ANIMATOR_ROOT_PATH) and that is the binding a
    # clip's extracted root motion arrives on -- the travel a humanoid solve splits out of
    # RootT/MotionT and hands over as position+rotation curves on that path. Filtering this
    # dict (or bone_name below) for truthiness is what silently dropped every character's
    # travel while leaving the in-place body motion looking correct.
    path_to_node = {n.path: n for n in nodes.values() if n.path is not None}
    ...
    for path in animated_paths:
        ...
        if bone_name is None or node is None:
            continue
...
def _write_bone_fcurves(fcurves, bone_name, frames, locs, quats, scales):
    ...
    bone of the rig like any other (the prefab's root node), so extracted root motion
    rides its pose channels and composes with the hips exactly as Unity composes the
    animator root with the body."""
