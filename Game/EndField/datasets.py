...
def projected_table(roots, container, column_specs, distinct_by="", prefer_non_empty=""):
    ...
    The root list and the spec list are both variadic, so how many are roots is
    STATED rather than inferred -- see the dataset's own rootCount parameter."""
    flat = []
    ...
    roots = list(roots)
    return _table(TABLE, container, distinct_by, prefer_non_empty, len(roots), *roots, *flat)


# ── scenes ──────────────────────────────────────────────────────────────────

def scene_maps(roots):
    return _column(MAPS, "map", *roots)
...
def landmarks(roots):
    return [{"level_id": row["levelId"],
            ...
            for row in _rows(LANDMARKS, *roots)]


def chunk_summary(roots, map_name):
    counts = _rows(CHUNK_SUMMARY, map_name, *roots)
    row = counts[0] if counts else {}
    return {
        "scene_state_ids": [_int(state) for state in _column(SCENE_STATES, "sceneState", map_name, *roots)],
...
def placements(roots, map_name, min_x, min_z, max_x, max_z, scene_state_id, lod0_only):
    ...
    window once."""
    args = (map_name, min_x, min_z, max_x, max_z, scene_state_id,
            "1" if lod0_only else "0", *roots)
    rows = _rows(PLACEMENTS, *args)
    ...
    for material in _rows(PLACEMENT_MATERIALS, *args):
        index = _int(material["placement"])
        if 0 <= index < len(rows):
            rows[index]["material_asset_paths"].append(material["path"])

    counts = _rows(PLACEMENT_COUNTS, *args)
    count = counts[0] if counts else {}
    return {
        "placements": rows,
        "seed_paths": _column(SEED_PATHS, "path", *args),
...
def npc_manifest(roots):
    return set(_column(NPC_MANIFEST, "template", *roots))


def npc_parts(roots, template_id):
    ...
    rows = _rows(NPC_PARTS, template_id, *roots)
...
def npc_materials(roots, template_id, cabs):
    """{mesh name: [material container path]} for one template."""
    assigned = {}
    for text in _mono_behaviour_texts(cabs):
        for row in _rows(NPC_MATERIALS, template_id, text, *roots):
...
def character_models(cabs):
    ...
    return {row["characterId"]: {"model": row["model"], "tag": row["tag"], "asset": row["asset"]}
            for row in _rows(CHARACTER_MODELS, *texts)}
