...
class RURI_OT_story_choose(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        from . import stage
        gated = stage.gate_branches(context, script.option, script.last_option)
...
def play_if_ready(context):
    """Play the built story, when there is one to play.

    Asked BEFORE calling: playing needs a beat script to advance through, and a
    ...
    if not bpy.ops.ruri.story_play.poll():
        return False
